      ...
      <main style={S.main}>
        ...
        {view === "users" && (<div>
          ...
          <div style={S.card}><div style={S.tw}><table style={S.tbl}><thead><tr>
            ...
            {users.map(u => {
              const isProtectedRow = u.name === SUPER_ADMIN_NAME || u.role === SUPER_ADMIN_ROLE;
              return (
                <tr key={u.name} style={!u.active ? { opacity: 0.4 } : {}}>
                  <td style={{ ...S.td, fontWeight: 600 }}>
                    ...
                    {u.role === SUPER_ADMIN_ROLE && (
                      <span style={{
                        ...
                        background: 'rgba(232,146,42,0.18)', color: C.orange,
                  ...
                  <td style={S.td}>
                    {editingRoleFor?.name === u.name ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button onClick={() => handleRoleChange(u.name, editingRoleValue)} style={{ padding: '4px 10px', borderRadius: 5, border: 'none', background: C.green, color: C.white, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>
                        <button onClick={() => setEditingRoleFor(null)} style={{ padding: '4px 10px', borderRadius: 5, border: `1px solid ${C.border}`, background: 'transparent', color: C.textMuted, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                      </div>
                    ) : (
                      <span>{u.role}</span>
                    )}
                  ...
                  <td style={S.td}>
                    <span style={{ ...S.badge, background: u.active ? C.green : C.textDim }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {editingRoleFor?.name !== u.name && (
                          <button
                            ...
                            style={{ padding: '4px 10px', borderRadius: 5, border: `1px solid ${C.accentBorder}`, background: 'transparent', color: C.accent, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}
                          >Edit Role</button>
                        )}
                        <button onClick={async () => {
