/**
 * CCOAN WAREHOUSE TRACKER - GOOGLE APPS SCRIPT BACKEND V6.1
 * Last updated: 2026-06-06 (CCOAN New York)
 *
 * CHANGES IN V6.1:
 * - uploadPhoto() now returns lh3.googleusercontent.com URL for reliable image embedding
 * - Added changePin() — allows users to change their own 4-digit PIN
 * - Added setUserActive case to doPost switch (was missing)
 * - Added Super Admin protection to deleteUser() — Samuel cannot be deleted
 * - Added Super Admin protection to setUserActive() — Samuel cannot be deactivated
 *
 * CHANGES IN V6:
 ...
 * - Added updateUserRole() — inline role editing from Users tab
 ...
 * N=PhotoURL | O=CreatedAt
 *
 * SHEET COLUMN STRUCTURE (Users):
 * A=Name | B=Role | C=PIN | D=Active
 */
...
function doPost(e) {
  try {
    ...
    switch (data.action) {
      ...
      case "updateUserRole": return makeResponse(...updateUserRole(data.userName, data.newRole));
      case "setUserActive":  return makeResponse(...setUserActive(data.userName, data.active));
      case "changePin":      return makeResponse(...changePin(data.userName, data.currentPin, data.newPin));
...
function uploadPhoto(fileName, mimeType, base64Data) {
  try {
    ...
    return [true, { photoUrl: `https://lh3.googleusercontent.com/d/${file.getId()}` }];
...
function updateUserRole(userName, newRole) {
  if (!userName || !newRole)
    return [false, { message: "userName and newRole are required." }];

  // Protect Super Admin — role cannot be changed externally
  if (String(userName).trim() === "Samuel")
    return [false, { message: "Super Admin role cannot be changed." }];
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === String(userName).trim().toLowerCase()) {
      ...
      return [true, { message: `${userName} updated to ${newRole}` }];
    }
  }
  return [false, { message: "User not found: " + userName }];
}

function changePin(userName, currentPin, newPin) {
  if (!userName || !currentPin || !newPin)
    return [false, { message: "All fields required." }];

  const p = String(newPin).trim();
  if (p.length !== 4 || isNaN(p))
    return [false, { message: "New PIN must be 4 digits." }];
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim().toLowerCase() === String(userName).trim().toLowerCase()) {
      if (String(data[i][2]).trim() !== String(currentPin).trim())
        return [false, { message: "Current PIN is incorrect." }];
      sheet.getRange(i + 1, 3).setValue(String(newPin));
      return [true, { message: "PIN updated successfully." }];
    }
  }
...
function setUserActive(userName, active) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).toLowerCase() === String(userName).toLowerCase()) {
      sheet.getRange(i + 1, 4).setValue(active === true || active === "true");
      return [true, { message: (active ? "Activated: " : "Deactivated: ") + userName }];
    }
  }
  return [false, { message: "User not found: " + userName }];
}
